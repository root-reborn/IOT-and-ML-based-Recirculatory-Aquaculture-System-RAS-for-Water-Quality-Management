//This code is for running our RAS system in Active Mode.We will take readings from Sensors and Predict Dissolved Oxygen from ML model and Send this data to cloud via IOT module and Run Actuators only if the values of water quality parameters are abnormal(Active Mode).
//Components like aerator will run indefinitely.But we can also make it run only when there is abnormality in the Dissolved Oxygen(which we predicted using ML model).
...
if (tur>10 || tds>250){            //Here we are specifying threshold values of turbidity and tds values(which depend upon the choosen fish breed in our RAS system) and the water pump will pump water from fish tank to mechanical filter for purification only if any of these threshold values is crossed.
    lcd.print("led ON              ");
    ...
    // Do Nothing i.e Water Pump which pumps water from Fish Tank to Mechanical Filter will not turn on since water quality parameters(i.e tds and turbidity) are not abnormal.
 }
...
if (ph>8.5){              //Here we are specifying the threshold value of ph(which depends upon the choosen fish breed) and the water pump will pump water from fish tank to biological filter for purification only if this threshold is crossed.
    ...
    lcd.clear();
     ...
     // Do Nothing i.e Water Pump which pumps water from Fish Tank to Biological Filter will not turn on since water quality parameters(i.e ph) is not abnormal.
 }
}
